...
class CriterionBoundary(nn.Module):
    '''
    DSN : We need to consider two supervision for the model.
    '''
    def __init__(self, ignore_index=255, use_weight=True, reduction='mean'):
        super(CriterionBoundary, self).__init__()
        self.ignore_index = ignore_index
        self.criterion = torch.nn.CrossEntropyLoss(ignore_index=ignore_index, reduction=reduction)
        if not reduction:
            print("disabled the reduction.")

    def forward(self, preds, target, boundary_target):
        h, w = target.size(1), target.size(2)

        if len(preds) >= 2:
            scale_pred = F.interpolate(input=preds[0], size=(h, w), mode='bilinear', align_corners=True)
            scale_boundary_pred = F.interpolate(input=preds[1], size=(h, w), mode='bilinear', align_corners=True)
            loss1 = self.criterion(scale_pred, target)
            loss1_boundary = self.criterion(scale_boundary_pred, boundary_target)

            scale_pred = F.interpolate(input=preds[2], size=(h, w), mode='bilinear', align_corners=True)
            scale_boundary_pred = F.interpolate(input=preds[3], size=(h, w), mode='bilinear', align_corners=True)
            loss2 = self.criterion(scale_pred, target)
            loss2_boundary = self.criterion(scale_boundary_pred, boundary_target)
            return loss1 + loss2 * 0.4 + loss1_boundary * 0.2 + loss2_boundary * 0.2 * 0.4
        else:
            scale_pred = F.interpolate(input=preds[0], size=(h, w), mode='bilinear', align_corners=True)
            scale_boundary_pred = F.interpolate(input=preds[1], size=(h, w), mode='bilinear', align_corners=True)
            loss = self.criterion(scale_pred, target)
            loss_boundary = self.criterion(scale_boundary_pred, boundary_target)
            return loss + loss_boundary * 0.2