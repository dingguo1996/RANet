...
from loss.criterion import CriterionDSN, CriterionOhemDSN, CriterionBoundary
...
def main():
    ...
    with Engine(custom_parser=parser) as engine:
        ...
        if args.ohem:
            ...
            criterion = CriterionBoundary() #CriterionCrossEntropy()
        ...
        while run:
            ...
            for idx in pbar:
                ...
                images, labels, boundary_label, _, _ = dataloader.next()
                images = images.cuda(non_blocking=True)
                labels = labels.long().cuda(non_blocking=True)
                boundary_labels = boundary_label.long().cuda(non_blocking=True)

                optimizer.zero_grad()
                lr = adjust_learning_rate(optimizer, args.learning_rate, global_iteration-1, args.num_steps, args.power)
                loss = model(images, labels, boundary_labels)
