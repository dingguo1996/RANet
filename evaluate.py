...
def predict_multiscale(net, image, tile_size, scales, classes, flip_evaluation, recurrence):
    ...
    for scale in scales:
        ...
        scaled_probs = ndimage.zoom(scaled_probs, (1., 1. / scale, 1. / scale, 1.),
...
def main():
    ...
    with Engine(custom_parser=parser) as engine:
        ...
        for idx in pbar:
            image, label, boundary_label, size, name = dataloader.next()
            size = size[0].numpy()
            with torch.no_grad():
                output = predict_multiscale(model, image, input_size, [1.0], args.num_classes, False, 0) #[0.5, 0.75, 1.0, 1.25, 1.5]
